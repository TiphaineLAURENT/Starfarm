...
namespace Catch
{
  ...
  std::size_t listReporters()
  {
          ...
          IReporterRegistry::FactoryMap const &factories = getRegistryHub().getReporterRegistry()
                                                                           .getFactories();
...
namespace Catch
{
  ...
  namespace
  {
    ...
    IStreamingReporterPtr makeReporter(std::shared_ptr<Config> const &config)
    {
            ...
            auto const &listeners = Catch::getRegistryHub().getReporterRegistry()
                                                           .getListeners();
...
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
          const auto &exceptions = getRegistryHub().getStartupExceptionRegistry()
                                                   .getExceptions();
          if (!exceptions.empty()) {
  ...
  int Session::runInternal()
  {
          ...
          CATCH_TRY {
                  ...
                  return (std::min) (
                          MaxExitCode, (std::max) (
                                  totals.error, static_cast<int>(totals.assertions
                                                                       .failed)));
          }
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
          catch (std::exception &ex) {
...
// Formatter impl for ConsoleReporter
    ...
    private:
            ...
            void printReconstructedExpression() const
            {
                    if (result.hasExpandedExpression()) {
                            ...
                            stream
                                    << Column(result.getExpandedExpression()).indent(
                                            2
                                    )
                                    << '\n';
                    }
  ...
  void ConsoleReporter::benchmarkStarting(BenchmarkInfo const &info)
  {
          ...
          auto nameCol = Column(info.name).width(
                  static_cast<std::size_t>(m_tablePrinter->columnInfos()[0].width
                                           - 2 ));
